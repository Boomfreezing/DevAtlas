...
- GitHub 导入限制仓库根地址、重定向、超时与下载大小，只下载默认分支，不执行仓库代码；每次跳转都在发出下一次请求前校验 HTTPS 与 GitHub 允许域名，响应声明或实际传输超过上限都会提前终止。短暂连接、读取和 5xx 故障最多自动重试一次，限流响应遵守 GitHub 返回的等待时间而不连续重试。
- GitHub 网络请求明确读取 HTTPX 支持的 `HTTP_PROXY`、`HTTPS_PROXY`、`ALL_PROXY` 与 `NO_PROXY` 进程环境变量；代理连接失败、连接超时、传输停滞、限流和服务端故障会返回不同处理建议。下载后的 ZIP 与本地文件夹采用一致的扩展名、依赖目录、文件数量、单文件和总大小过滤策略。
...
本项目采用 [MIT License](./LICENSE)。你可以使用、修改和分发代码，但须保留原版权声明和许可证文本。项目按现状提供，不附带任何明示或默示担保。
