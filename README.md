...
| 修改影响分析 | 选择文件、类或函数后，聚合定义位置、直接调用者、依赖对象、二级影响、相关测试、接口、数据库实体和循环依赖；生成风险评分与 P1/P2/P3 修改验证清单 |
| 分析快照 | 保存结构、质量和依赖指标；对比两次分析中的新增、已修复与持续问题；GitHub 项目可查看默认分支、HEAD 与最近提交，不复制仓库源码或 `.git` 历史 |
...
CI 在每次 `push` 和 Pull Request 时分别执行 Ruff 后端静态检查、后端测试、前端测试与 e2e 流程。Playwright 测试覆盖从本地文件夹导入，到报告生成、Markdown 下载、BM25 搜索和无变化增量分析的核心用户路径。
