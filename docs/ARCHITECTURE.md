...
影响结果进一步转换为按 P1/P2/P3 排序的行动清单：优先运行或补充相关测试，随后检查直接调用者、接口与数据契约、依赖环和二级影响，最后重新分析并保存快照。每项建议只引用已定位的项目文件，不生成无法验证的测试命令。

当前 GitHub 接入仍通过默认分支 ZIP 完成安全下载，不保留 `.git` 对象；同时通过 GitHub 只读 API 将仓库地址、默认分支、HEAD 和最近 20 条提交保存到独立 Git 元数据表。元数据请求失败不会阻断源码导入，用户也可在快照页手动刷新。两个完整提交 SHA 可通过 GitHub Compare API 获取真实远端差异，响应仅保留有界的文件路径、状态和增删行统计，不保存补丁正文。远端提交差异与本地源码快照相互独立，界面不会暗示本地副本已同步；任意分支、长期热点和共同变更仍是后续边界。

远程源码同步采用双目录提升策略。后台任务先读取远端 HEAD，与已保存 HEAD 相同则立即结束；出现新提交时，源码 ZIP 在新的受管目录中完成解压安全检查、扫描、结构解析、质量计算和搜索索引构建。只有暂存分析全部成功后，数据库才在一个事务中将新的文件、符号、导入和解析结果迁移回原项目 ID，并保留既有快照、增加 `REMOTE_SYNC` 快照。事务提交后再清理旧目录；暂存阶段失败时删除新目录，原项目数据与源码保持不变。
...
SQLite 用于保存项目、文件元数据、内容哈希、后台任务、符号、导入关系、解析问题、搜索片段、分析快照和轻量 GitHub 提交元数据，使分析结果可以在重启后继续使用。原始仓库文件仍保存在本地目录中，数据库不承担大文件对象存储。该组合无需额外部署数据库服务，适合个人电脑和简历演示。
